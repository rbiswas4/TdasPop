...
class SersicSamples(object):
    """
    Class for sampling sersic profiles of galaxies

    The idea is that the radial probability profile of light given by 
    L(r) / L(infinity), where L(r) =  \int d\phi \int dr r I(r) 
    This is then \frac{\gamma(2n, x)}{\Gamma(2n)} where  x =  (r/alpha)^{1/n}
    """
    ...
    def sampleRadius(self, halfLightRadius, numSamples=1, sersicIndex=1):
        ...
        sersicIndex. The answers are for sersic index of 1 and 4 only. The
        use of the halfLight radius is strictly correct for circular galaxies
        and might need a minor correction like the geometric mean of major and
        minor axes which has not been investigated. A useful reference is 
        Graham 2005.
